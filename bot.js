...
var tweetSearchSpanish = { q: '@LearnSpanish', count: 10, result_type: 'recent' };
var tweetSearchChinese = { q: '@ChinesePod', count: 10, result_type: 'recent' };
...
function tweetEvent(tweet) {
  ...
  if (reply_to === 'soydesauce') {
    // Get rid of the @ mention
    txt = txt.replace(/@soydesauce/g, '');
...
function retweetLatestSpanish() {
  T.get('search/tweets', tweetSearchSpanish, function(error, data) {
...
function retweetLatestChinese() {
  T.get('search/tweets', tweetSearchChinese, function(error, data) {
    var tweets = data.statuses;
    for (var i = 0; i < tweets.length; i++) {
      console.log(tweets[i].text);
    }
...
retweetLatestSpanish();
retweetLatestChinese();
// ...and then every hour after that. Time here is in milliseconds, so
// 1000 ms = 1 second, 1 sec * 60 = 1 min, 1 min * 60 = 1 hour --> 1000 * 60 * 60
setInterval(retweetLatestSpanish, 1000 * 60 * 12);
setInterval(retweetLatestChinese, 1000 * 60 * 12);
